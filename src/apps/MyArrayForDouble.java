...
public class MyArrayForDouble {
   ...
   public void add(double val){ // WOrst case: full, 0(elements.length), // avg
        if (isFull()) { //if full
    ...
    private void enlarge() { // doubles up the size of elements // 0(elements.length)
        double[] newArray = new double[elements.length * 2]; // makes a new array equal to the original with double the space (improves Perf)
    ...
    public int search(double val){ // output of search is...
        return linearSearch(val); // ...the output of this function
        // return binarySearch(val);
    }
    private int linearSearch(double val){ // best case 0(1), worse 0(numElements)
        for(int i=0;i<numElements;i++)
            if(elements[i] == val)
                return i;
        return -1; // -1 returned if not found
    }
    public int remove (double val){   // best case 0(1), worse 0(numElements)
