...
public class MyArrayForString {
   ...
   public void add(String val){ // WOrst case: full, 0(elements.length), // avg
        if (isFull()) { //if full
    ...
    private void enlarge() { // doubles up the size of elements // 0(elements.length)
        String[] newArray = new String[elements.length * 2]; // makes a new array equal to the original with double the space (improves Perf)
    ...
    public int search(String val){ // output of search is...
        return linearSearch(val); // ...the output of this function
        // return binarySearch(val);
    }
    private int linearSearch(String val){ // best case 0(1), worse 0(numElements)
        for(int i=0;i<numElements;i++)
            if(elements[i] == val)
                return i;
        return -1; // -1 returned if not found
    }
    public int remove (String val){   // best case 0(1), worse 0(numElements)
    ...
    public String min(){
        ...
        if (isEmpty())
            return null;
        String minV = elements[0];
        for(int i = 1; i<numElements; i++)
            if(elements[i].compareTo(minV) < 0) // checks elements of array vs minV value
    ...
    public void sort(){
        ...
        for(int i = 0; i< numElements; i++) { //0(numElements^2)
            ...
            for(int j = 1; j< numElements; j++)
                if (elements[j].compareTo(elements[minIdx]) < 0)
                    minIdx = j;
